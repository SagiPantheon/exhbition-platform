...
const QUICK_ACTIONS = [
  { icon: "⊞", label: "Add Exhibition", labelHe: "הוסף תערוכה" },
  { icon: "◈", label: "Add Exhibit",    labelHe: "הוסף מוצג"   },
  { icon: "⊡", label: "Add Supplier",   labelHe: "הוסף ספק"    },
  { icon: "≡", label: "Generate Report",labelHe: "הפק דוח"     },
  { icon: "↓", label: "Export Dashboard",labelHe: "יצא דשבורד" },
]

const TABLE_HEADERS = {
  en: ["Exhibition","Location","Dates","Theme","Type","Budget","Guests","Status"],
  he: ["תערוכה","מיקום","תאריכים","נושא","סוג","תקציב","אורחים","סטטוס"],
}
...
function StatusBadge({ status, he = false }: { status: ExhibitionStatus; he?: boolean }) {
...
function StatCard({ value, label, labelHe }: { value:string|number; label:string; labelHe:string; color:string }) {
  return (
    <div style={{ flex:1, minWidth:130, padding:"18px 20px", borderRadius:16,
      border:`1px solid rgba(34,211,238,0.2)`, background:`radial-gradient(ellipse at top,rgba(34,211,238,0.1),transparent 70%), rgba(8,16,36,0.9)` }}>
      <p style={{ margin:0, fontSize:28, fontWeight:900, color:"#22d3ee", lineHeight:1 }}>{value}</p>
    ...
    <div style={{ minHeight:"100vh", background:"#020818", color:"#fff",
      ...
      <header style={{ background:"rgba(4,12,30,0.97)", borderBottom:"1px solid rgba(0,200,255,0.15)",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, minWidth:220 }}>
          ...
          <div>
            <p style={{ margin:0, fontSize:13, fontWeight:800, color:"#f1f5f9", letterSpacing:"0.02em" }}>
              {he ? "מרכז ניהול תערוכות" : "Exhibition Command Center"}
            </p>
            <p style={{ margin:0, fontSize:10, color:"rgba(0,200,255,0.6)", letterSpacing:"0.08em" }}>
              {he ? "תמונת מצב שנתית וניהול תערוכות" : "Annual Overview & Management Dashboard"}
            </p>
          </div>
        </div>
        ...
        <nav style={{ flex:1, display:"flex", justifyContent:"center", gap:4 }}>
          {NAV_TABS.map((t, i) => (
            <button key={i} onClick={() => setActiveTab(i)} style={{
              ...
              <span>{he ? t.labelHe : t.label}</span>
            </button>
          ))}
        </nav>
      </header>
      ...
      <div style={{ flex:1, padding:"20px 24px", display:"flex", flexDirection:"column", gap:16, overflow:"auto" }}
        ...
        <div style={{ display:"flex", gap:12, alignItems:"stretch" }}>
          ...
          <div style={{ width:160, flexShrink:0, borderRadius:16, border:"1px solid rgba(0,200,255,0.2)",
            ...
            <p style={{ margin:"6px 0 3px", fontSize:10, fontWeight:700, color:"#22d3ee", letterSpacing:"0.1em", textTransform:"uppercase" }}>
              {he ? "נוכחות גלובלית" : "Global Presence"}
            </p>
        ...
        <div style={{ borderRadius:16, border:"1px solid rgba(0,200,255,0.12)",
          background:"rgba(6,12,28,0.8)", padding:"16px 20px" }}>
          <p style={{ margin:"0 0 14px", fontSize:11, fontWeight:700, color:"rgba(0,200,255,0.7)",
            letterSpacing:"0.18em", textTransform:"uppercase" }}>
            {he ? "ציר זמן תערוכות 2026" : "EXHIBITIONS TIMELINE 2026"}
          </p>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
            {(he ? MONTHS_HE : MONTHS).map(m => (
        ...
        {/* ══════════════════════ TABLE ══════════════════════ */}
        <div style={{ borderRadius:16, border:"1px solid rgba(0,200,255,0.12)",
          background:"rgba(6,12,28,0.8)", overflow:"hidden" }}>
          <div style={{ padding:"12px 16px", borderBottom:"1px solid rgba(0,200,255,0.1)",
            display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <p style={{ margin:0, fontSize:11, fontWeight:700, color:"rgba(0,200,255,0.7)", letterSpacing:"0.18em", textTransform:"uppercase" }}>
              {he ? "סקירת תערוכות" : "EXHIBITIONS OVERVIEW"}
            </p>
            <div style={{ display:"flex", gap:6 }}>
              {(["all","israel","abroad"] as const).map(r => (
                <button key={r} onClick={() => setRegionFilter(r)} style={{
                  ...
                  {r==="all" ? (he ? "הכל" : "All") : r==="israel" ? "🇮🇱 IL" : "✈️ Int'l"}
                </button>
              ))}
            </div>
          </div>
          ...
          {displayed.map(ex => (
            <div key={ex.id} onClick={() => setSelected(ex)}
              ...
              <StatusBadge status={ex.status} he={he} />
            </div>
          ))}
          <div style={{ padding:"8px 16px", borderTop:"1px solid rgba(0,200,255,0.08)" }}>
            <span style={{ fontSize:10, color:"rgba(0,200,255,0.5)", cursor:"pointer", letterSpacing:"0.1em" }}>
              {he ? "צפה בכל התערוכות ↙" : "View All Exhibitions →"}
            </span>
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1.2fr 1.6fr 1fr 0.9fr", gap:12 }}>
          ...
          <div style={{ borderRadius:16, border:`1px solid ${STATUS_CFG[selected.status].color}35`,
            ...
            <div style={{ position:"relative", height:110, overflow:"hidden" }}>
              ...
              <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:"8px 14px",
                ...
                <p style={{ margin:0, fontSize:14, fontWeight:800, color:"#fff" }}>
                  {he ? selected.nameHe : selected.nameEn}
                </p>
            ...
            <div style={{ padding:"14px" }}>
              <p style={{ margin:"0 0 10px", fontSize:10, letterSpacing:"0.2em", textTransform:"uppercase",
                color:"rgba(0,200,255,0.55)", fontWeight:700 }}>
                {he ? "פרטי תערוכה נבחרת" : "SELECTED EXHIBITION DETAILS"}
              </p>
              {[
                ["📅", he?"תאריכים":"Dates",        `${new Date(selected.startDate).toLocaleDateString("en-GB",{day:"2-digit",month:"short"})} – ${new Date(selected.endDate).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}`],
                ["◈",  he?"נושא":"Theme",            selected.theme],
                ["⊛",  he?"סוג":"Type",              selected.requestType.charAt(0).toUpperCase()+selected.requestType.slice(1)],
                ["👤", he?"אחראי":"Responsible",     selected.responsible],
                ["🏢", he?"קבלן":"Contractor",       selected.contractor],
                ["👥", he?"אורחים משוערים":"Est. Guests", selected.guestOrigins.reduce((s,g)=>s+g.estimatedCount,0).toLocaleString()],
                ["💰", he?"תקציב משוער":"Est. Budget", fmt(selected.estimatedCostUSD)],
              ].map(([icon,label,val])=>(
              ...
              <div style={{ marginTop:10 }}>
                <StatusBadge status={selected.status} he={he} />
              </div>
            </div>
          </div>
          ...
          <div style={{ borderRadius:16, border:"1px solid rgba(0,200,255,0.12)",
            ...
            <p style={{ margin:"0 0 12px", fontSize:11, fontWeight:700, color:"rgba(0,200,255,0.7)",
              letterSpacing:"0.18em", textTransform:"uppercase" }}>
              {he ? "מוצגים מוקצים" : "ASSIGNED EXHIBITS"}
            </p>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {exhibits.map((ex,i) => {
                const sc = exhibitStatusColor[ex.status]
                return (
                  <div key={i} style={{ borderRadius:12, overflow:"hidden", position:"relative", height:200, background:"rgba(0,10,30,1)" }}>
                    <img src={ex.img} alt={ex.name}
                      style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"contain", objectPosition:"center" }} />
                    <div style={{ position:"absolute", inset:0,
                      background:"linear-gradient(to top, rgba(2,6,18,0.95) 0%, rgba(2,6,18,0.3) 60%, transparent 100%)" }} />
                    <div style={{ position:"absolute", bottom:0, left:0, right:0,
                      padding:"8px 10px", display:"flex", alignItems:"flex-end", justifyContent:"space-between", gap:6 }}>
                      <p style={{ margin:0, fontSize:12, fontWeight:700, color:"#f1f5f9", lineHeight:1.3,
                        flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        {he ? ex.nameHe : ex.name}
                      </p>
                      <span style={{ flexShrink:0, padding:"2px 8px", borderRadius:999,
                        fontSize:9, fontWeight:700, color:sc, background:`${sc}25`, border:`1px solid ${sc}55` }}>
                        {ex.status}
                      </span>
                    </div>
                  </div>
          ...
          <div style={{ borderRadius:16, border:"1px solid rgba(0,200,255,0.12)",
            ...
            <p style={{ margin:"0 0 10px", fontSize:11, fontWeight:700, color:"rgba(0,200,255,0.7)",
              letterSpacing:"0.18em", textTransform:"uppercase" }}>
              {he ? "פרופיל אורחים" : "GUEST PROFILE"}
            </p>
            <div style={{ height:140 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v:any,_:any,p:any)=>[`${v}%`, he ? p.payload.nameHe : p.payload.name]}
                    contentStyle={{ background:"#0a1628", border:"1px solid rgba(0,200,255,0.3)",
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
              {GUEST_PROFILE.map(g => (
                <div key={g.name} style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  <span style={{ fontSize:10, color:"rgba(255,255,255,0.6)", flex:1 }}>{he ? g.nameHe : g.name}</span>
          ...
          <div style={{ borderRadius:16, border:"1px solid rgba(0,200,255,0.12)",
            ...
            {[
              { type:"Professional", typeHe:"מקצועי", icon:"🏛", active:selected.requestType==="professional" },
              { type:"Academic",     typeHe:"אקדמי",   icon:"🎓", active:selected.requestType==="academic"     },
              { type:"Internal",     typeHe:"פנימי",   icon:"🏢", active:selected.requestType==="internal"     },
              { type:"Government",   typeHe:"ממשלתי",  icon:"⚖️", active:false                                 },
              { type:"Mixed",        typeHe:"מעורב",   icon:"⊛",  active:selected.requestType==="mixed"        },
            ].map(r => (
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1.2fr", gap:12 }}>
          ...
          <div style={{ borderRadius:16, border:"1px solid rgba(251,191,36,0.2)",
            ...
            <p style={{ margin:"0 0 12px", fontSize:11, fontWeight:700, color:"rgba(251,191,36,0.7)",
              letterSpacing:"0.18em", textTransform:"uppercase" }}>
              {he ? "התראות" : "ALERTS & NOTIFICATIONS"}
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {ALERTS.map((a,i) => (
                <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", padding:"10px 12px",
                  ...
                  <p style={{ margin:0, flex:1, fontSize:12, color:"#e2e8f0", lineHeight:1.4 }}>
                    {he ? a.textHe : a.text}
                  </p>
