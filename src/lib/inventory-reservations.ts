...
type ItemLike = { id: string; quantity: number }
...
export function buildInventoryUsageMap(
  ...
  return new Map(
    items.map((item) => {
      ...
      const reserved = getReservedCountForInventory(exhibitions, item.id)
      const available = Math.max(0, total - reserved)
      return [item.id, { inventoryId: item.id, total, reserved, available }]
    })
  )
}
